...
def test_generator_is_an_iterator2():
    ...
    try:
        ...
        print "No more Objects to iterate"
        assert True
...
def test_generator_range_does_not_allocate_memory():
    ...
    assert True # did you reach here without any memory exception?
     #No memory exception because the code doesnt allocate memory for anything
...
-How generators are used and knowing the difference between a function and a generator.
-How yield acts and how generators can be used with for loops.
-next() is used to keep moving the function forward and at every stop point where you.
have a yield or return the function.
-Code that collects all results from the generator into a list. :)
"""

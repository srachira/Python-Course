...
notes = '''
In python lambda's are used to define simple anonymous single expression functions.
They are commonly used to pass small functions as parameters to higher order functions
(ie) functions which take other functions as parameters or return functions.
...
def test_lambda_has_access_to_locals():
    value = 10
    assert 20 == func_invoker(lambda x: x+value, 10)#Value of x is 10 and value=10 so 20 :)
...
-The lambda function are used to define simple functions.They pass arguments to higher order functions.
-Higher order functions are functions which take other functions as parameters or return functions.
-How to create and use lambda in a function.. :)
"""

...
def test_classes_bound_and_unbound_methods():
    ...
    assert q1.push.__self__ == q1_push.__self__      #bound method
...
-Classes are callable objects just like function objects.
-When a function appears within the body of a class statement it is transformed
into an unbound method. When a function is accessed on a class instance, it is
transformed into a bound method.
-Class may or may not be used in python, there is no compulsion like in other languages.
We only use classes where necessary.. :) :)
"""

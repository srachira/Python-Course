...
def test_int_iterable():
    try:
        ...
        print "Int object is not iterable"
                                       # replace by appropriate except so this test passes
...
def test_convert():
    ...
    try:
        ...
        print "Expected String, Int found"
        assert True
...
def test_find_builtins_that_work_on_iterables():
    funcs = [all,any,dict,filter,frozenset,list,map,max,min,reduce,set,sorted,sum,tuple,zip]
    assert 15 == len(funcs)


three_things_i_learnt = """
-an iterator is an object which allows a programmer to traverse through all
the elements of a collection, regardless of its specific implementation.
-next() is used to fetch the next element
-tuples,list,sets,dicts use the iter concept.
"""

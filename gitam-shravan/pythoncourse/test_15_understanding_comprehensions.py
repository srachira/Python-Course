...
def test_sc_nested():
    result = { x+y for x in range(3) for y in range(3)}
    assert 5 == len(result)
    assert set([0, 1, 2, 3, 4])== result #Set doesnt return duplicate values
...
-Comprehensions are used to create lists,dicts,sets in an easy way
-Using the list comprehension taking support of other functions
-Dictionary comprehension,creates a dict from some source of data
-Use of "ord"-- ord(c) it returns the integer ordinal of a one-character string.
Basically the ascii value.
-Set comprehensions are very similar to dict comprehensions except that
they deal a single value and create set objects. :)
"""
